...
public class blindness implements ISpell {
    @Override
    public void cast(int level, EntityPlayerMP player, boolean entityDetected, int entityID) {
        if (entityDetected) {
            ...
            if (target.isEntityAlive()) {
                ...
                targetLiving.addPotionEffect(new PotionEffect(Potion.getPotionById(15), SimpleSorceryUtils.getTicksFromSeconds(5 * level)));
