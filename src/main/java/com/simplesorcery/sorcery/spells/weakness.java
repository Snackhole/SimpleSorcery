...
public class weakness implements ISpell {
    @Override
    public void cast(int level, EntityPlayerMP player, boolean entityDetected, int entityID) {
        if (entityDetected) {
            ...
            if (target.isEntityAlive()) {
                ...
                targetLiving.addPotionEffect(new PotionEffect(Potion.getPotionById(18), SimpleSorceryUtils.getTicksFromMinutes(level), level - 1));
