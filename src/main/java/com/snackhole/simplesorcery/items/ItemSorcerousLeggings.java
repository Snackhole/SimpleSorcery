...
public class ItemSorcerousLeggings extends ItemArmorBase {
    public ItemSorcerousLeggings(String name) {
        super(SimpleSorceryMain.sorcerousArmorMaterial, EntityEquipmentSlot.LEGS, name);
    }
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        if (playerIn.isSneaking()) {
            ...
            if (!worldIn.isRemote) {
                ...
                if (itemDamage != 0) {
                    int cost = (int) Math.min(10, Math.ceil((double) itemDamage / (double) 6));
                    ...
                    if (cost <= currentPlayerFood) {
                        ...
                        playerIn.sendMessage(SimpleSorceryUtils.sorceryMessage("You must eat more food to repair these leggings!"));
                    }
                }
            }
