...
public class absorption implements ISpell {
    @Override
    public void cast(int level, EntityPlayerMP player, boolean entityDetected, int entityID) {
        player.addPotionEffect(new PotionEffect(Potion.getPotionById(22), SimpleSorceryUtils.getTicksFromMinutes(5 * level), level - 1));
    }
}
