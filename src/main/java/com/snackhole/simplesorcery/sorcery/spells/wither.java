...
public class wither implements ISpell {
    @Override
    public void cast(int level, EntityPlayerMP player, boolean entityDetected, int entityID) {
        if (entityDetected) {
            ...
            if (target.isEntityAlive()) {
                ...
                targetLiving.addPotionEffect(new PotionEffect(Potion.getPotionById(20), SimpleSorceryUtils.getTicksFromSeconds(5 * level), level - 1));
