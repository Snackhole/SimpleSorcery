...
public class strength implements ISpell {
    @Override
    public void cast(int level, EntityPlayerMP player, boolean entityDetected, int entityID) {
        player.addPotionEffect(new PotionEffect(Potion.getPotionById(5), SimpleSorceryUtils.getTicksFromMinutes(level), level - 1));
    }
}
